...
def upgrade():
    ...
    for counter, item in enumerate(items):
        conn.execute(sa.update(
            organization).where(organization.c.id == item.id).values(uuid=uuid1mc_from_datetime(item.created_at)
            ))
    ...
    for counter, item in enumerate(items):
        conn.execute(sa.update(
            team).where(team.c.id == item.id).values(uuid=uuid1mc_from_datetime(item.created_at)
            ))
        progress.update(counter)
    progress.finish()
    ...
    for counter, item in enumerate(items):
        conn.execute(sa.update(
            user).where(user.c.id == item.id).values(uuid=uuid1mc_from_datetime(item.created_at)
            ))
        progress.update(counter)
    progress.finish()
    ...
    for counter, item in enumerate(items):
        conn.execute(sa.update(
            useroldid).where(useroldid.c.userid == item.userid).values(uuid=item.uuid)
            )
        progress.update(counter)
    progress.finish()
