...
def upgrade():
    op.create_table('name',
        ...
        sa.CheckConstraint(u'CASE WHEN (user_id IS NOT NULL) THEN 1 ELSE 0 END + CASE WHEN (org_id IS NOT NULL) THEN 1 ELSE 0 END + CASE WHEN (reserved = true) THEN 1 ELSE 0 END = 1', name='username_owner_check'),
    ...
    op.execute(sa.DDL('''CREATE UNIQUE INDEX ix_name_name_lower ON "name" (lower(name) varchar_pattern_ops);'''))
...
def downgrade():
    op.add_column(u'user', sa.Column('username', sa.VARCHAR(length=80), autoincrement=False, nullable=True))
    op.create_unique_constraint(u'user_username_key', 'user', ['username'])
    op.add_column(u'organization', sa.Column('name', sa.VARCHAR(length=80), autoincrement=False, nullable=True))
    ...
    op.execute(sa.DDL('''CREATE INDEX ix_user_username_lower ON "user" (lower(username) varchar_pattern_ops);'''))
